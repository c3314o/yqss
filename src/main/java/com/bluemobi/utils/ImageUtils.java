...
public class ImageUtils {
	 ...
	 * 
	 */
	public static Image saveImage(MultipartFile imageFile, boolean needSmallImage) throws Exception {
		...
		if (null != imageFile && imageFile.getOriginalFilename().toLowerCase().trim().length() != 0) {
			...
			String webpath = (ImageUtils.class.getResource("/").getPath().substring(0,
					(ImageUtils.class.getResource("/").getPath().indexOf("webapps") + 7))) + File.separator;
			// 获取项目在磁盘上面的物理路径
			...
			imageSize = getSize(webpath + path);
			if (needSmallImage) {
				String suoluepath = imageSuoluePath + timeMillis + "_" + rand.nextInt(1000000) + originalFileName;
				try {
					FileCopyUtils.copy(imageFile.getBytes(), image);
		...
		_image.setImage(paths[0]);
		_image.setSmallUrl(paths[1]);
		_image.setWidth(imageSize[0]);
		_image.setHeight(imageSize[1]);
		return _image;
	}
...
//			String webpath = (ImageUtils.class.getResource("/").getPath().substring(0,
//					ImageUtils.class.getResource("/").getPath().indexOf("WEB-INF")));
